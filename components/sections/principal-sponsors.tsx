...
export function PrincipalSponsors() {
  ...
  const SectionTitle = ({
    ...
    return (
      <h3
        className={`relative ${cormorant.className} text-xs sm:text-sm md:text-base lg:text-lg font-semibold uppercase text-[#6C6D71] mb-1.5 sm:mb-2 md:mb-3 tracking-[0.1em] sm:tracking-[0.15em] ${textAlign} ${className} transition-all duration-300`}
      >
        {children}
      </h3>
    )
  ...
  const NameItem = ({ name, align = "center" }: { name: string, align?: "left" | "center" | "right" }) => {
    ...
    return (
      ...
      >
        ...
        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-[#AAABB0]/18 to-transparent opacity-0 group-hover/item:opacity-100 transition-opacity duration-300 rounded-md" />

        <p
          className={`relative text-[#6C6D71] text-[9px] sm:text-[10px] md:text-xs lg:text-sm font-semibold leading-snug break-words ${textAlign} group-hover/item:text-[#8C8D92] transition-all duration-300`}
        >
          {name}
        </p>
      </div>
  ...
  return (
    <div ref={sectionRef}>
      <Section
        id="sponsors"
        className="relative py-12 md:py-16 lg:py-20 overflow-hidden bg-[#6C6D71]"
      >
        ...
        <div className="absolute inset-0 pointer-events-none overflow-hidden">
          ...
          />
          {/* Overlay with palette colors */}
          <div className="absolute inset-0 bg-gradient-to-b from-[#6C6D71]/60 via-[#8C8D92]/50 to-[#AAABB0]/40" />
        </div>
        ...
        >
          {/* Small label */}
          <p
            className={`${cormorant.className} text-[0.7rem] sm:text-xs md:text-sm uppercase tracking-[0.28em] text-[#E3E4E6] mb-2`}
          ...
          <h2
            className="style-script-regular text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-[#E3E4E6] mb-1.5 sm:mb-3 md:mb-4"
          ...
          <p className={`${cormorant.className} text-xs sm:text-sm md:text-base text-[#C7C8CC] max-w-2xl mx-auto leading-relaxed px-2 mt-2 mb-3`}>
          ...
          <div className="flex items-center justify-center gap-2 mt-3 sm:mt-4">
            <div className="w-8 sm:w-12 md:w-16 h-px bg-gradient-to-r from-transparent via-[#AAABB0]/80 to-transparent" />
            <div className="w-1.5 h-1.5 rounded-full bg-[#C7C8CC]/80" />
            <div className="w-8 sm:w-12 md:w-16 h-px bg-gradient-to-l from-transparent via-[#AAABB0]/80 to-transparent" />
          </div>
        </div>
        ...
        >
          ...
          <div className="relative bg-[#E3E4E6]/95 backdrop-blur-md rounded-xl sm:rounded-2xl overflow-hidden border border-[#C7C8CC]/60 shadow-[0_20px_60px_rgba(108,109,113,0.35)] transition-all duration-500 group">
            {/* Card content */}
              ...
              {isLoading ? (
                <div className="flex items-center justify-center py-24 sm:py-28 md:py-32">
                  <div className="flex flex-col items-center gap-4">
                    <Loader2 className="h-10 w-10 sm:h-12 sm:w-12 animate-spin text-[#8C8D92]/70" />
                    <span className="text-[#6C6D71]/80 font-serif text-base sm:text-lg">Loading sponsors...</span>
                  </div>
                </div>
              ) : error ? (
                <div className="flex items-center justify-center py-24 sm:py-28 md:py-32">
                  <div className="text-center">
                    <p className="text-[#8C8D92] font-serif text-base sm:text-lg mb-3">{error}</p>
                    <button
                      onClick={fetchSponsors}
                      className="text-[#6C6D71]/90 hover:text-[#6C6D71] font-serif underline transition-colors duration-200"
                    >
                      Try again
                    </button>
                  </div>
                ...
                <div className="text-center py-24 sm:py-28 md:py-32">
                  <Users className="h-14 w-14 sm:h-16 sm:w-16 text-[#AAABB0]/40 mx-auto mb-4" />
                  <p className="text-[#6C6D71]/75 font-serif text-base sm:text-lg">No sponsors yet</p>
                </div>
              ) : (
